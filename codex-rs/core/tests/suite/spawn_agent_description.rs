...
async fn spawn_agent_description_lists_visible_models_and_reasoning_efforts() -> Result<()> {
    ...
    assert!(
        description.contains(
            "A spawned agent inherits your current model only when `model` is omitted. Set `model` explicitly whenever a different model is more appropriate for that child's work."
        ),
